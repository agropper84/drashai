'use client';
// Plan 2 file card. Two variants picked by the per-workflow defaultView
// (overridable on the list page). Sticky-note hover provides quick capture.
...
interface FileCardProps {
  encounter: Encounter;
  view: CardView;
  /** Override workflow's showSpine. Used by the list page's quick toggle. */
  showSpine?: boolean;
...
export function FileCard(props: FileCardProps) {
  const { view, encounter } = props;
  if (view === 'minimal') return <MinimalCard {...props} />;
  return <DetailedCard {...props} />;
}

// ─── Detailed (V3) ──────────────────────────────────────────
function DetailedCard({ encounter, showSpine, onSpark, onInsight, onTask }: FileCardProps) {
...
// ─── Minimal (V4 — envelope) ────────────────────────────────
function MinimalCard({ encounter, onSpark, onInsight, onTask }: FileCardProps) {
  const router = useRouter();
  const { getById: getTemplate } = useTemplates();
  const { getByTemplate } = useWorkflows();
  const tpl = getTemplate(encounter.type);
    ...
    <div className="card-host">
      <div
        ...
        <div style={{
          ...
          textTransform: 'uppercase', color: 'var(--ink-3)',
          marginBottom: 14, marginRight: encounter.sealed ? 30 : 0,
        }}>
