'use client';
// Plan 4 — cards now derive their phase state from the same heuristics
// as the file detail. Includes Plan 3's overflow menu + confirm dialog.
...
function useCardMenu(encounter: Encounter, onArchived?: (f: Encounter) => void, onDeleted?: (f: Encounter) => void) {
  ...
  const archive = async () => { await api.encounters.archive(encounter.id); await refresh(); onArchived?.(encounter); };
  const unarchive = async () => { await api.encounters.unarchive(encounter.id); await refresh(); };
  ...
  const doDelete = async () => { await remove(encounter.id); onDeleted?.(encounter); setConfirmOpen(false); };
...
function DetailedCard({ encounter, showSpine, onSpark, onInsight, onTask, onArchived, onDeleted }: FileCardProps) {
  ...
  // Plan 4 — derive phase state from heuristics for consistency with detail view.
  const { current, completed } = wf
...
function MinimalCard({ encounter, onSpark, onInsight, onTask, onArchived, onDeleted }: FileCardProps) {
  ...
  const isDelivered = completed.includes(phases[phases.length - 1]);
