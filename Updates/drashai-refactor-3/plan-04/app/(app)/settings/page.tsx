'use client';
// Plan 4 — Workflow editor gains a phase→tab mapping panel.
// All other sections (Keys, Appearance, Account, Archived) unchanged.
  ...
  return (
    <div className="settings-section">
      ...
      <div style={{
        ...
        Each phase routes to a tab when clicked on the status spine.
      </div>
                  ...
                  {isEditing ? (
                    <input
                      ...
                      onChange={(e) => update(w.id, (prev) => {
                        const newPhases = prev.phases.map((x, j) => (j === i ? e.target.value : x));
                        const newMap: Partial<Record<string, FileTab>> = {};
                        prev.phases.forEach((oldName, j) => {
                          const mapped = prev.phaseTabMap?.[oldName];
        ...
        <div className="settings-row settings-row-stack">
          <div>
            ...
            <div className="settings-help">
              Files you archived from their card menu. Restore one, or erase all at once.
            </div>
          </div>
        </div>
